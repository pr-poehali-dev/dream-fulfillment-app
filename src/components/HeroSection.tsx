    ...
    <>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center px-4 text-center"
        ...
        <div className="animate-fade-in flex flex-wrap items-center justify-center gap-3"
          ...
          <button
            onClick={() => { setFindOpen(o => !o); setFindValue(''); setFindError(''); }}
            className="flex items-center gap-2 px-5 py-3 rounded-full text-sm font-golos glass-panel transition-all"
        ...
        {findOpen && (
          <div
            ...
            style={{
              position: 'absolute', bottom: 'calc(8% + 56px)', left: '50%', transform: 'translateX(-50%)',
              zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
            }}>
            <div className="font-golos" style={{ fontSize: 12, color: 'rgba(200,210,240,0.45)', letterSpacing: '0.04em' }}>
              Введите номер от 1 до <span style={{ color: '#c9a84c' }}>{starsCount > 0 ? starsCount : '...'}</span>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                ...
                value={findValue}
                onChange={e => { setFindValue(e.target.value); setFindError(''); }}
                onKeyDown={async e => {
                  if (e.key === 'Enter' && findValue) {
                    setFindLoading(true);
                    const result = await onFindStar(Number(findValue));
                    setFindLoading(false);
                    if (result === 'ok') { setFindOpen(false); setFindValue(''); }
                    else if (result === 'out_of_range') setFindError(`Введите номер от 1 до ${starsCount}`);
                    else setFindError('Что-то пошло не так');
                  }
                }}
                placeholder="№"
                style={{
                  width: 90, padding: '8px 14px', borderRadius: 99,
                  background: 'rgba(20,25,40,0.9)',
                  border: `1px solid ${findError ? 'rgba(255,80,80,0.5)' : 'rgba(201,168,76,0.3)'}`,
                  color: '#f0e8d0', fontFamily: '"Golos Text", sans-serif', fontSize: 14,
              ...
              <button
                ...
                style={{
                  ...
                  background: findValue && !findLoading ? 'rgba(201,168,76,0.15)' : 'rgba(201,168,76,0.05)',
                  border: '1px solid rgba(201,168,76,0.3)',
                  color: findValue && !findLoading ? '#c9a84c' : 'rgba(201,168,76,0.35)',
            ...
            {findError && (
              <div className="font-golos" style={{ fontSize: 12, color: 'rgba(255,100,100,0.85)' }}>
                {findError}
              </div>
            )}
          </div>
